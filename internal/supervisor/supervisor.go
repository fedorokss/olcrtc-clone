...
func runCycle(
	...
	for i := range profiles {
		if err := runProfile(ctx, cfg, run, state, cycle, i, &profiles[i], lastErr); err != nil {
			return err
		}
	}
	return nil
...
func newStatusTracker(profiles []Profile, historyLimit int, notify func(Status)) *statusTracker {
	...
	statusProfiles := make([]ProfileStatus, len(profiles))
	for i := range profiles {
		statusProfiles[i].Name = profiles[i].Name
	}
...
func (t *statusTracker) end(profileIndex, cycle int, err error) {
	...
	if err != nil {
		msg := err.Error()
		profile.Failures++
		profile.LastError = msg
		t.status.LastError = fmt.Sprintf("profile %q: %s", profile.Name, msg)
		event.Error = msg
	} else {
		profile.CleanEnds++
		profile.LastError = ""
...
func (t *statusTracker) appendHistory(event Event) {
	...
	h := append(t.status.History, event)
	if len(h) > t.historyLimit {
		h = h[len(h)-t.historyLimit:]
	}
	t.status.History = h
}
