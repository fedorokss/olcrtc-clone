...
const (
	...
	protocolMagic         uint32 = 0x4f564331
...
func New(ctx context.Context, cfg transport.Config) (transport.Transport, error) {
	...
	if tr.fragmentSize <= 0 {
		tr.fragmentSize = defaultFragmentSize
	}
	if tr.ackTimeout <= 0 {
		tr.ackTimeout = defaultAckTimeout
	}
	if tr.frameInterval <= 0 {
		tr.frameInterval = defaultFrameInterval
	}
	if tr.batchSize <= 0 {
...
func (p *streamTransport) Send(data []byte) error {
	...
	fragments := common.FragmentPayload(data, p.fragmentSize)
	...
	for attempt := range maxSendAttempts {
		if attempt == 0 {
			totalLen := len(data)
			fragTotal := len(fragments)
			for idx, fragment := range fragments {
				frame := encodeDataFrame(seq, crc, totalLen, idx, fragTotal, fragment)
...
func (p *streamTransport) perAttemptAckTimeout(fragments int) time.Duration {
	drainTicks := (fragments + p.batchSize - 1) / p.batchSize
	estimated := time.Duration(drainTicks) * p.frameInterval * 3
	if estimated < p.ackTimeout {
		return p.ackTimeout
	}
...
func encodeDataFrame(seq, crc uint32, totalLen, fragIdx, fragTotal int, payload []byte) []byte {
	...
	binary.BigEndian.PutUint32(out[14:18], uint32(totalLen))  //nolint:gosec,lll
	binary.BigEndian.PutUint16(out[18:20], uint16(fragIdx))   //nolint:gosec,lll
	binary.BigEndian.PutUint16(out[20:22], uint16(fragTotal)) //nolint:gosec,lll
	copy(out[22:], payload)
	return out
}
