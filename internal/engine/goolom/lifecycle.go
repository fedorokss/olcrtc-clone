...
func (s *Session) handleReconnectAttempt(ctx context.Context, maxReconnects int, reconnectWindow time.Duration) bool {
	now := time.Now()
	if now.Sub(s.lastReconnect) > reconnectWindow {
		s.reconnectCount = 0
	}
	s.reconnectCount++
	s.lastReconnect = now
...
func (s *Session) reconnect(ctx context.Context) error {
	...
	if err := s.sleepCtx(ctx, 500*time.Millisecond); err != nil {
		return err
	}
	s.stopSession()
	s.closeTransport()
	if err := s.sleepCtx(ctx, 3*time.Second); err != nil {
		return err
	}
...
func (s *Session) sleepCtx(ctx context.Context, d time.Duration) error {
	...
	case <-timer.C:
		return nil
	case <-s.closeCh:
		return fmt.Errorf("reconnect aborted: session closed")
	case <-ctx.Done():
