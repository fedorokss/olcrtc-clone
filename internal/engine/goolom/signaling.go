...
var (
	...
	conferenceEndKeys = [...]string{"conferenceClosed", "conferenceEnded", "roomClosed", "roomEnded", "callEnded"}
	ackMessageKeys    = [...]string{"updateDescription", "slotsConfig", "slotsMeta", "vadActivity"}
	endedStates       = [...]string{"closed", "ended", "finished"}
)

func (s *Session) sendHello() error {
	hasVideo := s.hasLocalVideoTracks()
...
func (s *Session) handleCommonMessages(msg map[string]any, uid string) {
	for _, key := range ackMessageKeys {
		if _, ok := msg[key]; ok {
			s.sendAck(uid)
		}
	}
...
func isConferenceEndMessage(msg map[string]any) bool {
	for _, key := range conferenceEndKeys {
		if _, ok := msg[key]; ok {
			return true
		}
	}
...
func isEndedState(state string) bool {
	...
	return strings.EqualFold(state, stateTerminated)
}
