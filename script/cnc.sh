...
PODMAN_ID=$(LC_ALL=C tr -dc 'a-z0-9' </dev/urandom 2>/dev/null | head -c 8)
...
if [ "$AUTH" = "jitsi" ]; then
    ...
    case "$JITSI_SERVER_CHOICE" in
        2) JITSI_BASE_URL="https://meet.cryptopro.ru" ;;
        3)
    ...
    case "$JITSI_ROOM_INPUT" in
        http://*|https://*|*/*) ROOM_ID="$JITSI_ROOM_INPUT" ;;
...
git clone --depth 1 --single-branch --recurse-submodules --shallow-submodules --branch "$BRANCH" "$REPO_URL" "$WORK_DIR"

echo "[*] Pulling Go image..."
podman image exists "$IMAGE_NAME" || podman pull "$IMAGE_NAME"
