...
if [ "$CARRIER" = "jitsi" ]; then
    echo ""
    echo "Р вЂ™РЎвЂ№Р В±Р ВµРЎР‚Р С‘РЎвЂљР Вµ Jitsi-РЎРѓР ВµРЎР‚Р Р†Р ВµРЎР‚ (Р С—РЎР‚Р С•Р Р†Р ВµРЎР‚РЎРЉРЎвЂљР Вµ Р Р† Р В±РЎР‚Р В°РЎС“Р В·Р ВµРЎР‚Р Вµ, Р С”Р В°Р С”Р С•Р в„– РЎР‚Р В°Р В±Р С•РЎвЂљР В°Р ВµРЎвЂљ Р Р† Р Р†Р В°РЎв‚¬Р ВµР в„– РЎРѓР ВµРЎвЂљР С‘):"
    echo "  1) https://meet1.arbitr.ru/"
    echo "  2) https://meet.cryptopro.ru/"
    echo "  3) Р вЂќРЎР‚РЎС“Р С–Р С•Р в„– (Р Р†Р Р†Р ВµРЎРѓРЎвЂљР С‘ Р Р†РЎР‚РЎС“РЎвЂЎР Р…РЎС“РЎР‹)"
    read -p "Р вЂ™Р Р†Р ВµР Т‘Р С‘РЎвЂљР Вµ Р Р…Р С•Р СР ВµРЎР‚ [1-3, Р С—Р С• РЎС“Р СР С•Р В»РЎвЂЎР В°Р Р…Р С‘РЎР‹: 1]: " JITSI_SERVER_CHOICE
    case "$JITSI_SERVER_CHOICE" in
        2)
            JITSI_BASE_URL="https://meet.cryptopro.ru"
            ;;
        3)
            read -p "Р вЂ™Р Р†Р ВµР Т‘Р С‘РЎвЂљР Вµ URL Jitsi-РЎРѓР ВµРЎР‚Р Р†Р ВµРЎР‚Р В°: " JITSI_BASE_INPUT
            JITSI_BASE_URL="${JITSI_BASE_INPUT%/}"
            if [ -z "$JITSI_BASE_URL" ]; then
                echo "[X] URL Р Р…Р Вµ Р СР С•Р В¶Р ВµРЎвЂљ Р В±РЎвЂ№РЎвЂљРЎРЉ Р С—РЎС“РЎРѓРЎвЂљРЎвЂ№Р С"
                exit 1
            fi
            ;;
        *)
...
if [ "$GEN_ROOM" = "1" ]; then
    if [ "$CARRIER" = "wbstream" ]; then
        echo "[*] WB Stream room will be generated by the server at startup."
        ROOM_ID=""
    else
...
if [ "$CARRIER" = "wbstream" ] && [ -n "$ROOM_ID" ]; then
    echo "$ROOM_ID" > "$ORIG_DIR/wb_stream_id"
    echo "[*] Saved WB Stream session to $ORIG_DIR/wb_stream_id"
elif [ "$CARRIER" = "wbstream" ]; then
    echo "[*] WB Stream session will be saved to wb_stream_id when the server starts."
fi
...
if [ -z "$ROOM_ID" ]; then
    OLC_URI="olcrtc://$CARRIER?${TRANSPORT}${TRANSPORT_PAYLOAD}@[SEE_WB_STREAM_ID_FILE]#$KEY\$$sub_configname"
else
...
echo "  cd $WORK_DIR && $BIN_PATH server.yaml"
echo ""
